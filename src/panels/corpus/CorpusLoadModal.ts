...
export type CorpusLoadSelection =
  | {
      library: "static";
      libraryId: string;
      entry: StaticCorpusLibrary["entries"][number];
    }
  | { library: "user"; id: string; label: string };

export interface CorpusLoadModalOptions {
  onSelect: (selection: CorpusLoadSelection) => void;
}

export class CorpusLoadModal {
  private readonly backdrop: HTMLDivElement;
  private readonly list: HTMLDivElement;
  private readonly status: HTMLParagraphElement;
  private readonly onSelect: CorpusLoadModalOptions["onSelect"];
  private readonly onKeyDown: (event: KeyboardEvent) => void;
  private cachedStaticLibraries: StaticCorpusLibrary[] = [];
  private open = false;

  constructor(options: CorpusLoadModalOptions) {
    ...
    dialog.setAttribute("aria-labelledby", "corpus-load-title");
    dialog.className =
    ...
    title.id = "corpus-load-title";
    title.textContent = "Load corpus";
    title.className = "m-0 text-sm font-semibold text-slate-100";

    const subtitle = document.createElement("p");
    subtitle.textContent = "Choose a corpus from a library.";
  ...
  async openModal(): Promise<void> {
    ...
    try {
      const [staticLibraries, userEntries] = await Promise.all([
        loadAllStaticCorpusLibraries(),
        Promise.resolve(listUserCorpora()),
      ]);
  ...
  private refreshList(): void {
    this.renderEntries(this.cachedStaticLibraries, listUserCorpora());
  }

  private renderEntries(
    staticLibraries: StaticCorpusLibrary[],
    ...
    if (staticLibraries.length === 0 && userEntries.length === 0) {
      ...
      empty.textContent = "No corpora are available.";
      empty.className =
  ...
  private createUserLibraryRow(id: string, label: string): HTMLDivElement {
    ...
    deleteButton.title = "Delete saved corpus";
