export interface CorpusSaveModalOptions {
  getName: () => string;
  onSave: (name: string) => void;
}

export class CorpusSaveModal {
  private readonly backdrop: HTMLDivElement;
  private readonly input: HTMLInputElement;
  private readonly status: HTMLParagraphElement;
  private readonly getName: CorpusSaveModalOptions["getName"];
  private readonly onSave: CorpusSaveModalOptions["onSave"];
  private readonly onKeyDown: (event: KeyboardEvent) => void;
  private open = false;

  constructor(options: CorpusSaveModalOptions) {
    ...
    dialog.setAttribute("aria-labelledby", "corpus-save-title");
    dialog.className =
      "box-border w-full max-w-sm rounded-lg border border-slate-700 bg-slate-900 p-4 shadow-xl";

    const title = document.createElement("h2");
    title.id = "corpus-save-title";
    title.textContent = "Save corpus";
    title.className = "m-0 text-sm font-semibold text-slate-100";

    const subtitle = document.createElement("p");
    subtitle.textContent = "Save the current corpus to your local library.";
    ...
    this.input.placeholder = "Corpus name";
    this.input.className =
      "w-full rounded-md border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-teal-500 focus:outline-none";
    ...
    saveButton.className =
      "cursor-pointer rounded-md border border-teal-500 bg-teal-600 px-3 py-2 text-xs font-medium text-white hover:bg-teal-500";
  ...
  private submit(): void {
    ...
    if (!name) {
      this.status.textContent = "Enter a name to save this corpus.";
      return;
    }
