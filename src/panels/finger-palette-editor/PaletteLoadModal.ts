...
} from "../../kbts/keyboard/paletteLibrary.js";

export type PaletteLoadSelection =
  | {
      library: "static";
      libraryId: string;
      entry: StaticPaletteLibrary["entries"][number];
    }
  | { library: "user"; id: string; label: string };

export interface PaletteLoadModalOptions {
  onSelect: (selection: PaletteLoadSelection) => void;
}

export class PaletteLoadModal {
  private readonly backdrop: HTMLDivElement;
  private readonly list: HTMLDivElement;
  private readonly status: HTMLParagraphElement;
  private readonly onSelect: PaletteLoadModalOptions["onSelect"];
  private readonly onKeyDown: (event: KeyboardEvent) => void;
  private cachedStaticLibraries: StaticPaletteLibrary[] = [];
  private open = false;

  constructor(options: PaletteLoadModalOptions) {
    ...
    dialog.setAttribute("aria-labelledby", "palette-load-title");
    dialog.className =
  ...
  async openModal(): Promise<void> {
    ...
    try {
      const [staticLibraries, userEntries] = await Promise.all([
        loadAllStaticPaletteLibraries(),
        Promise.resolve(listUserPalettes()),
      ]);
  ...
  private refreshList(): void {
    this.renderEntries(this.cachedStaticLibraries, listUserPalettes());
  }

  private renderEntries(
    staticLibraries: StaticPaletteLibrary[],
    ...
    if (staticLibraries.length === 0 && userEntries.length === 0) {
      ...
      empty.textContent = "No palettes are available.";
      empty.className =
  ...
  private createUserLibraryRow(id: string, label: string): HTMLDivElement {
    ...
    deleteButton.title = "Delete saved palette";
