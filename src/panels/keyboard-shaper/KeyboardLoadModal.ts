...
} from "../../kbts/keyboard/keyboardLibrary.js";

export type KeyboardLoadSelection =
  | {
      library: "static";
      libraryId: string;
      entry: StaticKeyboardLibrary["entries"][number];
    }
  | { library: "user"; id: string; label: string };

export interface KeyboardLoadModalOptions {
  onSelect: (selection: KeyboardLoadSelection) => void;
}

export class KeyboardLoadModal {
  private readonly backdrop: HTMLDivElement;
  private readonly list: HTMLDivElement;
  private readonly status: HTMLParagraphElement;
  private readonly onSelect: KeyboardLoadModalOptions["onSelect"];
  private readonly onKeyDown: (event: KeyboardEvent) => void;
  private cachedStaticLibraries: StaticKeyboardLibrary[] = [];
  private open = false;

  constructor(options: KeyboardLoadModalOptions) {
    ...
    dialog.setAttribute("aria-labelledby", "kb-load-title");
    dialog.className =
    ...
    title.id = "kb-load-title";
    title.textContent = "Load keyboard";
    title.className = "m-0 text-sm font-semibold text-slate-100";

    const subtitle = document.createElement("p");
    subtitle.textContent = "Choose a shape from a library.";
  ...
  async openModal(): Promise<void> {
    ...
    try {
      const [staticLibraries, userEntries] = await Promise.all([
        loadAllStaticLibraries(),
        Promise.resolve(listUserKeyboards()),
      ]);
  ...
  private refreshList(): void {
    this.renderEntries(this.cachedStaticLibraries, listUserKeyboards());
  }

  private renderEntries(
    staticLibraries: StaticKeyboardLibrary[],
    ...
    if (staticLibraries.length === 0 && userEntries.length === 0) {
      ...
      empty.textContent = "No keyboard shapes are available.";
      empty.className =
  ...
  private createUserLibraryRow(id: string, label: string): HTMLDivElement {
    ...
    deleteButton.title = "Delete saved keyboard";
