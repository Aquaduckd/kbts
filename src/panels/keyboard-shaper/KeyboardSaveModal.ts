export interface KeyboardSaveModalOptions {
  getName: () => string;
  onSave: (name: string) => void;
}

export class KeyboardSaveModal {
  private readonly backdrop: HTMLDivElement;
  private readonly input: HTMLInputElement;
  private readonly status: HTMLParagraphElement;
  private readonly getName: KeyboardSaveModalOptions["getName"];
  private readonly onSave: KeyboardSaveModalOptions["onSave"];
  private readonly onKeyDown: (event: KeyboardEvent) => void;
  private open = false;

  constructor(options: KeyboardSaveModalOptions) {
    ...
    dialog.setAttribute("aria-labelledby", "kb-save-title");
    dialog.className =
      "box-border w-full max-w-sm rounded-lg border border-slate-700 bg-slate-900 p-4 shadow-xl";

    const title = document.createElement("h2");
    title.id = "kb-save-title";
    title.textContent = "Save keyboard";
    title.className = "m-0 text-sm font-semibold text-slate-100";

    const subtitle = document.createElement("p");
    subtitle.textContent = "Save the current shape to your local library.";
    ...
    this.input.placeholder = "Keyboard name";
    this.input.className =
      "w-full rounded-md border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-indigo-500 focus:outline-none";
    ...
    saveButton.className =
      "cursor-pointer rounded-md border border-indigo-500 bg-indigo-600 px-3 py-2 text-xs font-medium text-white hover:bg-indigo-500";
  ...
  private submit(): void {
    ...
    if (!name) {
      this.status.textContent = "Enter a name to save this keyboard.";
      return;
    }
